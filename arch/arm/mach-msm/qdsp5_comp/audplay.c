...
void dump_buffer_status(struct audio *audio)
{
	...
	MM_AUD_INFO("dump_buffer_status: head = %d, tail = %d\n",
		audio->out_head, audio->out_tail);
	for (ptr = 0; ptr < BUFCNT; ptr++) {
		frame = audio->out + ptr;
		MM_AUD_INFO("buffer %d: used = %d, offset = %d\n",
...
void audplay_adsp_write(uint16_t *dest, uint16_t *src, uint32_t wlen)
{
	...
	if (!(((uint32_t)dest & 1) || ((uint32_t)src & 1))) {
		...
		if ((uint32_t)dest & 1) {
			/* Dest is misaligned */
			MM_AUD_INFO("Odd alignment on dest pointer\n");
			while (--wlen) {
				tword = *src++;
			...
			/* Src is misaligned */
			MM_AUD_INFO("Odd alignment on src pointer\n");
			while (--wlen) {
...
void audplay_send_lp_data(struct audio *audio, unsigned needed)
{
	...
	if (atomic_read(&audio->image_swap)) {
		MM_AUD_ERR("audplay_send_lp_data: image swap\n");
		return;
	}
...
void adjust_data_offset(struct audio *audio, uint32_t discard_bytes)
{
	...
	MM_AUD_INFO("adjust_data_offset: before adjust\n");
	...
	while (1) {
		...
		if (count != 0) {
			...
			MM_AUD_INFO("recover %d bytes from buffer %d\n",
				count, audio->out_tail);
		}

		if (discard_bytes == 0) {
			MM_AUD_INFO("adjust_data_offset: discard_bytes = 0\n");
			break;
		} else if (audio->out_tail == audio->out_head) {
			MM_AUD_INFO("adjust_data_offset: tail = head\n");
			break;
		} else {
			audio->out_tail = decr(audio->out_tail);
			MM_AUD_INFO("adjust_data_offset: change out_tail to %d\n",
				audio->out_tail);
		}
	}
	...
	while (1) {
		frame = audio->out + idx;
		if (frame->used == 0xffffffff) {
			MM_AUD_INFO("adjust_data_offset: reset frame %d\n",
				idx);
			frame->used = 0;
			frame->offset = 0;
		}
	...
	MM_AUD_INFO("adjust_data_offset: after adjust\n");
	dump_buffer_status(audio);
	if (discard_bytes)
		MM_AUD_INFO("adjust_data_offset: %d bytes lost\n",
			discard_bytes);
}
