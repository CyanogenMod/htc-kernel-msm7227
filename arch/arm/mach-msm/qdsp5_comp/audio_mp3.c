...
static void audio_dsp_event(void *private, unsigned id, uint16_t *msg)
{
	...
	case AUDPP_MSG_STATUS_MSG: {
		...
		case AUDPP_DEC_STATUS_SLEEP:
			MM_AUD_INFO("mp3:decoder status: sleep \n");
			break;
		case AUDPP_DEC_STATUS_INIT: {
			MM_AUD_INFO("mp3:decoder status: init \n");
			audpp_cmd_cfg_adec_params(audio);
			break;
		}
		case AUDPP_DEC_STATUS_CFG:
			MM_AUD_INFO("mp3:decoder status: cfg \n");
			break;
		case AUDPP_DEC_STATUS_PLAY:
			MM_AUD_INFO("mp3:decoder status: play \n");
			break;
		default:
			MM_AUD_ERR("mp3:unknown decoder status \n");
			break;
		}
		break;
	}
	case AUDPP_MSG_CFG_MSG:
		if (msg[0] == AUDPP_MSG_ENA_ENA) {
			MM_AUD_INFO("audio_mp3_dsp_event: CFG_MSG ENABLE\n");
			...
			MM_AUD_INFO("audio_mp3_dsp_event: CFG_MSG DISABLE\n");
			audpp_avsync(audio->dec_id, 0);
			audio->running = 0;
		} else {
			MM_AUD_ERR("audio_mp3_dsp_event: CFG_MSG %d?\n", msg[0]);
		}
		break;
	...
	case AUDPP_MSG_PCMDMAMISSED:
		...
		break;
	default:
		MM_AUD_ERR("audio_mp3_dsp_event: UNKNOWN (%d)\n", id);
	}
...
static void audio_mp3_audmgr_cb(void)
{
	...
	if (audmgr->state == STATE_DISABLING && audio->enabled) {
		if (audmgr_disable_event_rsp(audmgr))
			MM_AUD_ERR("audio_mp3_audmgr_cb:"
...
static void audio_mp3_work_func(struct work_struct *work)
{
	...
	if (module->state == ADSP_STATE_DISABLED && audio->enabled) {
		...
		MM_AUD_INFO("audio_mp3_work_func: %d bytes discarded.\n",
				discard_bytes);
		auddec_dsp_config(audio, 0);
		rc = wait_event_interruptible(
				audio->audpp_wait,
		...
		if (rc < 0) {
			MM_AUD_ERR("audio_mp3_work_func: wait_event failed\n");
			return;
		}
		audpp_disable(audio->dec_id, audio);
		rc = msm_adsp_disable_event_rsp(module);
		if (rc < 0)
			MM_AUD_ERR("audio_mp3_work_func:"
...
static int __init audio_init(void)
{
	...
	if (!the_mp3_audio.data) {
		MM_AUD_ERR("audio_mp3: could not allocate DMA buffers\n");
		return -ENOMEM;
	}
