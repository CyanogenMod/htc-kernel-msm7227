		...
		unsigned idx, unsigned len)
{
	...
	if (atomic_read(&audio->image_swap)) {
		MM_AUD_ERR("audio_dsp_send_buffer: image swap\n");
		return -1;
	}
...
static void audio_work_func(struct work_struct *work)
{
	...
	if (atomic_read(&audio->image_swap)) {
		...
		if (rc < 0) {
			MM_AUD_ERR("%s: can't wait audpp notification\n", __func__);
			return;
		}
		audpp_disable(-1, audio);
		if (audmgr_disable_event_rsp(audmgr))
			MM_AUD_ERR("audio: audmgr_disable_event_rsp() failed\n");
	} else {
		if (audpp_enable(-1, audio_dsp_event, audpp_modem_event, audio))
			MM_AUD_ERR("audio: audpp_enable() failed\n");
	}
