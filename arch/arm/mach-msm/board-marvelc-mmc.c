/* linux/arch/arm/mach-msm/board-marvelc-mmc.c
...
static int __init marvelc_disablesdcard_setup(char *str)
{
...
__setup("board_marvelc.disable_sdcard=", marvelc_disablesdcard_setup);
...
static uint32_t marvelc_sdslot_switchvdd(struct device *dev, unsigned int vdd)
{
	int i;
 ...
 */
static struct embedded_sdio_data marvelc_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int marvelc_wifi_cd;		/* WIFI virtual 'card detect' status */
static void (*wifi_status_cb)(int card_present, void *dev_id);
static void *wifi_status_cb_devid;

static int
marvelc_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static unsigned int marvelc_wifi_status(struct device *dev)
{
	return marvelc_wifi_cd;
}

static struct mmc_platform_data marvelc_wifi_data = {
	.ocr_mask		= MMC_VDD_28_29,
	.status			= marvelc_wifi_status,
	.register_status_notify	= marvelc_wifi_status_register,
	.embedded_sdio		= &marvelc_wifi_emb_data,
};

int marvelc_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(marvelc_wifi_set_carddetect);
...
int marvelc_wifi_power(int on)
{
	...
	mdelay(100);
	gpio_set_value(MARVELC_GPIO_WIFI_EN, on);
	mdelay(100);
	return 0;
}
EXPORT_SYMBOL(marvelc_wifi_power);
...
int __init marvelc_init_mmc(unsigned int sys_rev)
{
	...
	id = PCOM_GPIO_CFG(MARVELC_GPIO_WIFI_EN, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),
	msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &id, 0);
	gpio_set_value(MARVELC_GPIO_WIFI_EN, 0);

	wifi_status_cb = NULL;

	printk(KERN_INFO "%s\n", __func__);

	msm_add_sdcc(1, &marvelc_wifi_data, 0, 0);

	register_msm_irq_mask(INT_SDC2_0);
	register_msm_irq_mask(INT_SDC2_1);
	if (opt_disable_sdcard) {
		printk(KERN_INFO "marvelc: SD-Card interface disabled\n");
		goto done;
	}
	...
	set_irq_wake(MSM_GPIO_TO_INT(MARVELC_GPIO_SDMC_CD_N), 1);

	msm_add_sdcc(2, &marvelc_sdslot_data, MSM_GPIO_TO_INT(MARVELC_GPIO_SDMC_CD_N),
