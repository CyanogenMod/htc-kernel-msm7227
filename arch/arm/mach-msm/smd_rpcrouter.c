...
static struct msm_rpc_reply_buff rpc_reply_buff[MAX_REPLY] = {
	...
	[0 ... (MAX_REPLY-1)].cid = 0,
	[0 ... (MAX_REPLY-1)].xid = 0xffffffff
};

static spinlock_t reply_lock;

int save_pending_reply(uint32_t pend_pid, uint32_t pend_cid, uint32_t pend_xid)
{
	int i = 0;
	unsigned long flags;

	spin_lock_irqsave(&reply_lock, flags);
	while (i < MAX_REPLY) {
		if (rpc_reply_buff[i].pid == 0xffffffff) {
			rpc_reply_buff[i].pid = pend_pid;
			rpc_reply_buff[i].cid = pend_cid;
			rpc_reply_buff[i].xid = pend_xid;
...
int check_pending_reply(uint32_t *hdr_dst_pid, uint32_t *hdr_dst_cid, uint32_t rq_xid)
{
	int i = 0;
	unsigned long flags;
	...
	spin_lock_irqsave(&reply_lock, flags);
	while (i < MAX_REPLY) {
		if ((rpc_reply_buff[i].xid == rq_xid)) {
			*hdr_dst_pid = rpc_reply_buff[i].pid;
			*hdr_dst_cid = rpc_reply_buff[i].cid;
			rpc_reply_buff[i].pid = 0xffffffff; /*Clear Buffer*/
			rpc_reply_buff[i].xid = 0xffffffff;
			spin_unlock_irqrestore(&reply_lock, flags);
			return 0;
		}
		i++;
...
int msm_rpc_write(struct msm_rpc_endpoint *ept, void *buffer, int count)
{
	...
	if (rq->type == 0) {
		...
		if (ept->reply_xid != rq->xid) {
			printk(KERN_WARNING
					"rr_write: request_xid : %d != reply_xid : %d\n",
					be32_to_cpu(rq->xid), be32_to_cpu(ept->reply_xid));
			if (check_pending_reply(&hdr.dst_pid, &hdr.dst_cid, rq->xid)) {
				printk(KERN_ERR
					"rr_write: rejecting packet w/ bad xid\n");
				return -EINVAL;
			}
			printk(KERN_WARNING
				"ept->reply_pid = %d, ept->reply_cid = %d \n"
				"hdr.dst_pid = %d. hdr.dst_cid = %d\n",
				ept->reply_pid, ept->reply_cid,
				hdr.dst_pid, hdr.dst_cid);
		/*Do not consume this reply since we send previous pending reply.*/
		} else {
		   ...
		   unsigned len, long timeout)
{
	...
	*frag_ret = pkt->first;
	...
	if ((rc >= (sizeof(uint32_t) * 3)) && (rq->type == 0)) {
		...
		/* RPC CALL */
		if (ept->reply_pid != 0xffffffff) {
			if (save_pending_reply(ept->reply_pid, ept->reply_cid, ept->reply_xid)) {
				printk(KERN_WARNING"rr_read: Pending reply buffer full!!\n");
			}
			printk(KERN_WARNING
			...
			be32_to_cpu(rq->xid));
			printk(KERN_WARNING
				"ept->reply_pid = %d. ept_reply_cid = %d\n",
				ept->reply_pid, ept->reply_cid);
		}
		/* TODO: locking? */
