/* linux/arch/arm/mach-msm/board-icong-wifi.c
*/
...
int icong_wifi_set_carddetect(int on);
int icong_wifi_get_mac_addr(unsigned char *buf);
...
static void *icong_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init icong_init_wifi_mem(void)
{
	int i;
...
static struct resource icong_wifi_resources[] = {
	[0] = {
		.name		= "bcm4329_wlan_irq",
		.start		= MSM_GPIO_TO_INT(ICONG_GPIO_WIFI_IRQ),
		.end		= MSM_GPIO_TO_INT(ICONG_GPIO_WIFI_IRQ),
#ifdef HW_OOB
...
static struct wifi_platform_data icong_wifi_control = {
	.set_power      = icong_wifi_power,
	.set_reset      = icong_wifi_reset,
	.set_carddetect = icong_wifi_set_carddetect,
	.mem_prealloc   = icong_wifi_mem_prealloc,
	.get_mac_addr	= icong_wifi_get_mac_addr,
	.dot11n_enable  = 1,
};

static struct platform_device icong_wifi_device = {
        .name           = "bcm4329_wlan",
        .id             = 1,
        .num_resources  = ARRAY_SIZE(icong_wifi_resources),
        .resource       = icong_wifi_resources,
        .dev            = {
                .platform_data = &icong_wifi_control,
        },
};

extern unsigned char *get_wifi_nvs_ram(void);

static unsigned icong_wifi_update_nvs(char *str)
{
...
int icong_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk("icong_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	icong_wifi_update_nvs("sd_oobonly=1\n");
#endif
	icong_wifi_update_nvs("btc_params80=0\n");
	icong_wifi_update_nvs("btc_params6=30\n");
	icong_init_wifi_mem();
	ret = platform_device_register(&icong_wifi_device);
	return ret;
}
