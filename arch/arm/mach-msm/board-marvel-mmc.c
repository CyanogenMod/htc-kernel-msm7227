/* linux/arch/arm/mach-msm/board-marvel-mmc.c
...
static int __init marvel_disablesdcard_setup(char *str)
{
...
__setup("board_marvel.disable_sdcard=", marvel_disablesdcard_setup);
...
static uint32_t marvel_sdslot_switchvdd(struct device *dev, unsigned int vdd)
{
	int i;
...
static unsigned int marvel_sdslot_status(struct device *dev)
{
	unsigned int status;

	status = (unsigned int) gpio_get_value(MARVEL_GPIO_SDMC_CD_N);
	return (!status);
}

#define MARVEL_MMC_VDD		(MMC_VDD_28_29 | MMC_VDD_29_30)

static unsigned int marvel_sdslot_type = MMC_TYPE_SD;

static struct mmc_platform_data marvel_sdslot_data = {
	.ocr_mask	= MARVEL_MMC_VDD,
	.status_irq	= MSM_GPIO_TO_INT(MARVEL_GPIO_SDMC_CD_N),
	.status		= marvel_sdslot_status,
	.translate_vdd	= marvel_sdslot_switchvdd,
	.slot_type	= &marvel_sdslot_type,
};
 ...
 */
static struct embedded_sdio_data marvel_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int marvel_wifi_cd;		/* WIFI virtual 'card detect' status */
static void (*wifi_status_cb)(int card_present, void *dev_id);
static void *wifi_status_cb_devid;

static int
marvel_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static unsigned int marvel_wifi_status(struct device *dev)
{
	return marvel_wifi_cd;
}

static struct mmc_platform_data marvel_wifi_data = {
	.ocr_mask		= MMC_VDD_28_29,
	.status			= marvel_wifi_status,
	.register_status_notify	= marvel_wifi_status_register,
	.embedded_sdio		= &marvel_wifi_emb_data,
};

int marvel_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(marvel_wifi_set_carddetect);
...
int marvel_wifi_power(int on)
{
	...
	mdelay(100);
	gpio_set_value(MARVEL_GPIO_WIFI_EN, on);
	mdelay(100);
	return 0;
}
EXPORT_SYMBOL(marvel_wifi_power);
...
int __init marvel_init_mmc(unsigned int sys_rev)
{
	...
	id = PCOM_GPIO_CFG(MARVEL_GPIO_WIFI_EN, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),
	msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &id, 0);
	gpio_set_value(MARVEL_GPIO_WIFI_EN, 0);

	wifi_status_cb = NULL;

	printk(KERN_INFO "%s\n", __func__);

	msm_add_sdcc(1, &marvel_wifi_data, 0, 0);

	register_msm_irq_mask(INT_SDC2_0);
	register_msm_irq_mask(INT_SDC2_1);
	if (opt_disable_sdcard) {
		printk(KERN_INFO "marvel: SD-Card interface disabled\n");
		goto done;
	}
	...
	set_irq_wake(MSM_GPIO_TO_INT(MARVEL_GPIO_SDMC_CD_N), 1);

	msm_add_sdcc(2, &marvel_sdslot_data, MSM_GPIO_TO_INT(MARVEL_GPIO_SDMC_CD_N),
